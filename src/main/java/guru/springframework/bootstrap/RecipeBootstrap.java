...
public class RecipeBootstrap implements ApplicationListener<ContextRefreshedEvent> {
    ...
    //these 3 are which we will add their data directry(they are also who can stand without others/independently)
    ...
    public void onApplicationEvent(ContextRefreshedEvent contextRefreshedEvent) {
        recipeRepository.saveAll(getRecipes());//saveAll takes an Iterable//and List is an Iterable
    }
    ...
    private List<Recipe> getRecipes() {
        ...
        Optional<UnitOfMeasure> teaspoonUomOptonal = unitOfMeasureRepository.findByDescription(("Teaspoon"));
        if(!teaspoonUomOptonal.isPresent()){
            throw new RuntimeException("Expected UOM NOt Found");
        }
        ...
        Optional<UnitOfMeasure> pintUomOptonal = unitOfMeasureRepository.findByDescription("Pint");
        if(!teaspoonUomOptonal.isPresent()){
            throw new RuntimeException("Expected UOM NOt Found");
        }
...
/*        Optional<UnitOfMeasure> pinchUomOptonal = unitOfMeasureRepository.findByDescription(("Pinch"));
        if(!teaspoonUomOptonal.isPresent()){
            throw new RuntimeException("Expected UOM NOt Found");
        }*/
        ...
        UnitOfMeasure teaSpoonUom = teaspoonUomOptonal.get();
        UnitOfMeasure dashUom = dashUomOptional.get();
        UnitOfMeasure pintUom = pintUomOptonal.get();
        UnitOfMeasure cupUom = cupUomOptional.get();
//        UnitOfMeasure pinchUom = pinchUomOptonal.get();
...
//1st-Recipe: Guac
        ...
        //needed for bi-directional - should be onoe method call
...
//        tacosNotes.setRecipe(tacosRecipe); //deleted b/c we are doing it inside .setNotes(Note note);
        tacosRecipe.setNotes(tacosNotes);

//weNotDoin: tacosRecipe.getIngredients().add(new Ingredient("Ancho Chili Powder", new BigDecimal(2), tableSpoonUom, tacosRecipe));
//b/c now we have .addIngredient(Ingredient ingredient)... we also dont have to pass the Recipe to Ingredient constructor itll know using this keyword inside .addIngredient(...)
